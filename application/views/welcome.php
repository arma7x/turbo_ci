<?php defined('BASEPATH') OR exit('No direct script access allowed');?>

<div class="container">
	<div class="row starter-template">
		<div class="col col-12">
			<h2 class="text-center text-primary"><?php echo isset($page_name) ? strtoupper($page_name) : strtoupper('Codeigniter') ;?></h2>
		</div>
		<div class="col col-12 col-lg-7 mb-4">
			<div class="row text-left justify-content-sm-center align-items-center">
				<div class="col col-12">
					<div class="card shadow-sm mb-4">
						<div class="card-body">
							<span class="text-primary">=></span> Fork <strong class="text-pink"><?php echo APP_NAME ?></strong> on <a href="https://github.com/arma7x/turbo_ci" target="_blank" rel="noopener">Github</a>. If you are exploring <strong>CodeIgniter</strong> for the very first time, you should start by reading the <a href="https://codeigniter.com/user_guide/" target="_blank" rel="noopener">User Guide</a>
						</div>
					</div>
				</div>
				<div class="col col-12 col-sm-6">
					<div class="card shadow-sm mb-4">
						<div class="card-body">
							<span class="text-primary">=></span> If you would like to edit this page you'll find it located at: <small><code>application/views/welcome.php</code></small>
						</div>
					</div>
				</div>
				<div class="col col-12 col-sm-6">
					<div class="card shadow-sm mb-4">
						<div class="card-body">
							<span class="text-primary">=></span> The corresponding controller for this page is found at: <small><code>application/controllers/Welcome.php</code></small>
						</div>
					</div>
				</div>
				<div class="col col-12 col-sm-6">
					<div class="card shadow-sm mb-4">
						<div class="card-body">
							<span class="text-primary">=></span> Login as <strong><?php echo lang('L_MODERATOR') ?><br></strong> <strong class="text-pink">ahmadmuhamad101@gmail.com</strong><br><strong class="text-pink">1111111111</strong> (1x10)
						</div>
					</div>
				</div>
				<div class="col col-12 col-sm-6">
					<div class="card shadow-sm mb-4">
						<div class="card-body">
							<span class="text-primary">=></span> Login as <strong><?php echo lang('L_MEMBER') ?><br>
							</strong> <strong class="text-pink">arma7x@live.com</strong><br><strong class="text-pink">1111111111</strong> (1x10)
						</div>
					</div>
				</div>
			</div>
			<span class="d-lg-none"><hr></span>
		</div>
		<div class="col col-12 col-lg-5">
			<div class="row justify-content-center">
				<div class="col col-12 col-sm-6 mb-4">
					<div class="card shadow-sm">
						<div class="card-body">
							<blockquote class="blockquote">
							  <p class="mb-0 text-pink"><strong>Written in PHP</strong></p>
							  <footer class="blockquote-footer">CodeIgniter is a powerful PHP framework with a very small footprint.</footer>
							</blockquote>
						</div>
					</div>
				</div>
				<div class="col col-12 col-sm-6 mb-4">
					<div class="card shadow-sm">
						<div class="card-body">
							<blockquote class="blockquote">
							  <p class="mb-0 text-pink"><strong>CSS Styling</strong></p>
							  <footer class="blockquote-footer">Bootstrap is the most popular HTML, CSS, and JavaScript framework.</footer>
							</blockquote>
						</div>
					</div>
				</div>
				<div class="col col-12 col-sm-6 mb-4">
					<div class="card shadow-sm">
						<div class="card-body">
							<blockquote class="blockquote">
							  <p class="mb-0 text-pink"><strong>Utilize Javascript</strong></p>
							  <footer class="blockquote-footer">JQuery to program the behavior of web pages and handle user interaction.</footer>
							</blockquote>
						</div>
					</div>
				</div>
				<div class="col col-12 col-sm-6 mb-4">
					<div class="card shadow-sm">
						<div class="card-body">
							<blockquote class="blockquote">
							  <p class="mb-0 text-pink"><strong>Fast Navigation</strong></p>
							  <footer class="blockquote-footer">Turbolinks® makes navigating your web application faster.</footer>
							</blockquote>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
